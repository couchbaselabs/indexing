...
func (kn *knode) insert(key Key, v Value) (Node, Node, bkey, []Node) {
    ...
    if kn.size <= kn.store.maxKeys() {
        return kn, nil, bkey{}, []Node{}
    }
    spawnKn, median := kn.split()
    return kn, spawnKn, median.(bkey), []Node{}
}

func (in *inode) insert(key Key, v Value) (Node, Node, bkey, []Node) {
    ...
    // Recursive insert
    child, spawn, median, stalenodes := child.insert(key, v)
    stalenodes = append(stalenodes, stalechild)
    in.vs[index] = child.getKnode().fpos
    if spawn == nil {
        return in, nil, bkey{}, stalenodes
    }
    ...
    return in, spawnIn, medianN.(bkey), stalenodes
}
...
func (kn *knode) split() (*knode, interface{}) {
    // Get a free block
